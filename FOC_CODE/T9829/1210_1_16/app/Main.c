...
int main(void)
{
  ...
  while (1u)                          //各个任务的执行周期不同，由执行周期将执行周期相同的函数放在一个位置
  {																		//MsCnt1每1ms在SysTick_Handler中加一次  系统中断的优先级较高
    if(MsCnt1 >= SCH_PRG_TIMER_BASE)  //MsCnt1初始值为0 SCH_PRG_TIMER_BASE == 5   该if每5ms进入一次 1 2 3 4 5-->0(执行if) 1 2 ...
    {
      MsCnt1 = 0u;
      ...
      if(MsCnt10 == 0u)
      {
        MsCnt10 = SCH_CNT_10MS_OVER;
				...
				PwmIn_PeriodTask();        //PWM输入检测
				ADCSample_PeriodTask();    //ADC检测
				SDK_EcuFaultDiag();        //错误诊断-赋值操作

				if(IsInTestMode == 0u)/* 2021-07-28-by zyg  */
				{
					Main_AppMainInputUpdate();  //输入更新
					AppMain_step();             //模型执行步骤(代码自动生成)
				  ...
				  MotorAPI_MotorSetSpeed(TargetSpeed);        //对限幅后的速度作用在电机上
					MotorAPI_MotorPeriodTask();                 //电机各种状态的应对措施
				}
