...
//x,y：起点坐标
//sizex,sizey,图片长宽       //***********   *:沿y轴画8个点
//BMP[]：要写入的图片数组
//mode:0,反色显示;1,正常显示
void LCD_ShowPicture(uint16_t x,uint16_t y,uint16_t sizex,uint16_t sizey,const uint8_t BMP[],uint16_t color)
{
    uint16_t j = 0;
    uint16_t i, n, temp, m;
    uint16_t x0 = x,y0 = y;
...
extern uint8_t AirQuality;  //气体质量值

extern uint8_t TemperatureMax;//温度阈值
extern uint8_t HumidityMax;   //湿度阈值
extern uint8_t LightMax;      //光照度阈值
extern uint8_t AirQualityMin; //气体质量阈值
////
extern uint8_t Lcd_Page;//LCD功能页面
extern uint8_t Windows_Status;//窗户打开标志
extern uint8_t LED1_Status;
extern uint8_t Curtain_Status;
extern uint8_t  BEEP1_Status;
extern uint8_t HomeMode;//0：手动模式 1：自动模式 
extern uint8_t changeflag;//页面切换清屏标志位

void LCD_ShowPage(uint8_t page)//LCD页面显示/切换函数
{
    if(page>=1 && page<=4)//一共有8个页面，第1~第4个页面作为一级页面，第5~第8个页面作为二级页面
    {
        lcd_show_string(150,80,200,24,24,"<-SmartHome->",RED);//90+60
        LCD_ShowPicture(60,210,48,48,left,0);//60  210  图片是从左上角开始画的
        LCD_ShowPicture(340,210,48,48,right,0);
    }
    if(page==1)//家电控制选项页面
    {
        //lcd_clear(WHITE);
        LCD_ShowPicture(160,160,128,128,Control,4);//160 288  160  288
        lcd_show_string(180,320,120,24,24,"Control",GREEN);
    }
    else if(page==2)//模式切换选项页面
    {
        //lcd_clear(WHITE);
        LCD_ShowPicture(160,160,128,128,Mode,2);
        lcd_show_string(180,320,120,24,24,"Mode",LIGHTBLUE);
    }
    else if(page==3)//阈值设置选项页面
    {
        LCD_ShowPicture(160,160,128,128,Setting,5);
        lcd_show_string(180,320,120,24,24,"Setting",BLUE);
    }
    else if(page==4)//传感器数据监测
    {
        LCD_ShowPicture(160,160,128,128,envir_data,6);
        lcd_show_string(180,320,120,24,24,"Sensor",MAGENTA);
    }
    
    else if(page==5)//进入控制后的功能控制页面
    {
        lcd_show_string(98,128,120,16,16,"1.LED_Sta :",RED);//+60+30  //+100
        if(LED1_Status==0)
        {
            lcd_show_string(240,128,24,16,16," on",GREEN);
            LCD_ShowPicture(300,128,32,32,On_Off,4);//300-332   128-160
        }
        else
        {
            lcd_show_string(240,128,24,16,16,"off",RED);
            LCD_ShowPicture(300,128,32,32,On_Off,0);
        }
        
        lcd_show_string(98,178,120,16,16,"2.Curtain_Sta :",RED);
        if(Curtain_Status==0)
        {
            lcd_show_string(240,178,24,16,16," on",GREEN);
            LCD_ShowPicture(300,178,32,32,On_Off,4);//300-332   178-210
        }
        else
        {
            lcd_show_string(240,178,24,16,16,"off",RED);
            LCD_ShowPicture(300,178,32,32,On_Off,0);
        }
        lcd_show_string(98,228,120,16,16,"3.Win_Sta :",RED);
        if(Windows_Status==0)
        {
            lcd_show_string(240,228,24,16,16," on",GREEN);
            LCD_ShowPicture(300,228,32,32,On_Off,4);//300-332   228-260
        }
        else
        {
            lcd_show_string(240,228,24,16,16,"off",RED);
            LCD_ShowPicture(300,228,32,32,On_Off,0);
        }
        lcd_show_string(98,278,120,16,16,"4.Beep_Sta :",RED);
        if(BEEP1_Status==0)
        {
            lcd_show_string(240,278,24,16,16," on",GREEN);
            LCD_ShowPicture(300,278,32,32,On_Off,4);//300-332   278-310
        }
        else
        {
            lcd_show_string(240,278,24,16,16,"off",RED);
            LCD_ShowPicture(300,278,32,32,On_Off,0);
        }
        
        LCD_ShowPicture(60,362,48,48,Back,0);
    }
    else if(page==6)//进入模式选择后的页面
    {
        //lcd_draw_rectangle(80,110,200,340, RED);//+60
        lcd_show_string(110,70,100,16,16,"AutoMode",RED);
        //lcd_draw_rectangle(240,110,360,340, GREEN);//画矩形
        lcd_show_string(268,70,100,16,16,"ManMode",RED);
        if(HomeMode==1)
            lcd_fill(80,110,200,340, GREEN);        //填充单色
        else
            lcd_fill(80,110,200,340, RED);        //填充单色
        if(HomeMode==0)
            lcd_fill(240,110,360,340, GREEN);        //填充单色
        else
            lcd_fill(240,110,360,340, RED);      //填充单色
        LCD_ShowPicture(60,362,48,48,Back,0);
    }
    else if(page==7)//进入Setting后的功能控制页面
    {
        lcd_show_string(98,128,120,16,16,"1.Mq135_Min: ",RED);//+90
        lcd_show_xnum(240,128,AirQualityMin,3,16,0,RED);
        LCD_ShowPicture(300,128,39,24,Add_Sub,0);
        
        lcd_show_string(98,178,120,16,16,"2.Temp_Max :",RED);
        lcd_show_xnum(240,178,TemperatureMax,3,16,0,RED);
        LCD_ShowPicture(300,178,39,24,Add_Sub,0);
        
        lcd_show_string(98,228,120,16,16,"3.Humi_Max :",RED);
        lcd_show_xnum(240,228,HumidityMax,3,16,0,RED);
        LCD_ShowPicture(300,228,39,24,Add_Sub,0);
        
        lcd_show_string(98,278,120,16,16,"4.Light_Max:",RED);
        lcd_show_xnum(240,278,LightMax,3,16,0,RED);
        LCD_ShowPicture(300,278,39,24,Add_Sub,0);
        
        LCD_ShowPicture(60,362,48,48,Back,0);
    }
    else if(page==8)//传感器数据监测
    {
        LCD_ShowPicture(140,118,32,32,AQ,4);//空气质量  //+90
        lcd_show_string(240,128,100,16,16,"airq",RED);
        lcd_show_xnum(300,128,AirQuality,3,16,0,BLACK);
        
		LCD_ShowPicture(140,168,32,32,Temp_bmp,1);//温度       
        lcd_show_string(240,178,100,16,16,"temp",RED);
        lcd_show_xnum(300,178,Temperature,3,16,0,BLACK);
        
        LCD_ShowPicture(140,218,32,32,Humi_bmp,2);//湿度
        lcd_show_string(240,228,100,16,16,"humi",RED);
        lcd_show_xnum(300,228,Humidity,3,16,0,BLACK);
        
        LCD_ShowPicture(140,268,32,32,Light_bmp,3);//关照强度
        lcd_show_string(240,278,100,16,16,"light",RED);
        lcd_show_xnum(300,278,Light,3,16,0,BLACK);
