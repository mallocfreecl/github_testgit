...
//用户名
#define PROID		"mqtt_stm32&k0ib4b2nX3P"
//密码
#define AUTH_INFO	"LUOBOCHUANGKE"
//产品ID
#define DEVID		"k0ib4b2nX3P"
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	cJSON *json , *json_value; //引入cJSON指针
	
	type = MQTT_UnPacketRecv(cmd);//MQTT解包看是什么种类
	switch(type)
	{
...
//				}
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				//对数据包req_payload进行JSON解析
				json = cJSON_Parse(req_payload);
				if(!json)UsartPrintf(USART_DEBUG,"Error before: [%s]\n",cJSON_GetErrorPtr());
				else
				{   //最开始的写法
//					json_value = cJSON_GetObjectItem(json , "LED_Sta");
//					if(json_value->valueint) //json_value > 0且为整型
//					{
//						LED1(0);//黄灯亮   //控制多个变量需要定义多个value取值
//					}
//					else
//					{
//						LED1(1);//黄灯熄灭			
//					}
					...
					UsartPrintf(USART_DEBUG,"json_value: [%s]\n",json_value->string);     //键的名称
					UsartPrintf(USART_DEBUG,"json_value: [%s]\n",json_value->valuestring);//键值
					if(strstr(json_value->valuestring,"LED")!= NULL)//json_value->string还是json_value->valuestring
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint) LED1(0);//黄灯亮
						else LED1(1);//黄灯熄灭
					}
					else if(strstr(json_value->valuestring,"BEEP")!= NULL)
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint) BEEP1(0);//蜂鸣器响:低电平触发
						else BEEP1(1);//蜂鸣器不响
					}
					else if(strstr(json_value->valuestring,"WINDOW")!= NULL)
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint) Servo_SetAngle(90);//舵机转90度
						else Servo_SetAngle(0);//舵机转0度
					}
					else if(strstr(json_value->valuestring,"CURTAIN")!= NULL)
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint) lastmove = 0;//步进电机正转360度
						else lastmove = 1;//步进电机反转360度
					}
