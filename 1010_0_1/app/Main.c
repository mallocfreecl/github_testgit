...
#define SCH_PRG_TIMER_BASE  	(25)    /*修改任务时基，24-06-19 by CL*/
#define SCH_CNT_25MS_OVER     (25/SCH_PRG_TIMER_BASE) /*修改10ms任务为25ms任务，24-06-19 by CL*/
...
static u16 MsCnt_25; /* 10ms count */
...
int main(void)
{
  ...
  while (1u)
  {
    if(MsCnt1 >= SCH_PRG_TIMER_BASE)
    {
      ...
      if(MsCnt_25 == 0u)/* polyspace DEFECT:USELESS_IF */
      {
				
        MsCnt_25 = SCH_CNT_25MS_OVER;
...
static void Main_FeedBackBusOutput(void)
{
	...
	{	
		...
		/*在不进行pwm捕获时使用上一次的占空比 24.11.13 by zlh*/
		U.PwmInCaptureEn = 0u;
		SDK_PwmOutHigh();
	...
	else
	{
		...
		U.PwmInCaptureEn = Y.FeedBackBusStatus;
	}
