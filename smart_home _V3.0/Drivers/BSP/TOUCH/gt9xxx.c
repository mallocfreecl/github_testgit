...
void gt9xxx_rd_reg(uint16_t reg, uint8_t *buf, uint8_t len)
{
    ...
    for (i = 0; i < len; i++)
    {
        buf[i] = ct_iic_read_byte(i == (len - 1) ? 0 : 1);  /* 读取数据 */ //最后一个字节要发非应答，令从机交给主机总线控制权
    }
...
uint8_t gt9xxx_scan(uint8_t mode)
{
    ...
    if ((t % 10) == 0 || t < 10)    /* 空闲时,每进入10次CTP_Scan函数才检测1次,从而节省CPU使用率 */ //if ((t % 10) == 0 || t < 10)
    {
        gt9xxx_rd_reg(GT9XXX_GSTID_REG, &mode, 1);  /* 读取触摸点的状态 */ //mode用于存放读取到的值，该寄存器的B7表示有触摸点
                                                                          //B0~B3(寄存器的最低4位表示有多少点同时按下)
        if ((mode & 0X80) && ((mode & 0XF) <= g_gt_tnum))                 //g_gt_tnum是全局变量且默认是5,gt9147最多同时支持5个点
        {
            i = 0;
            gt9xxx_wr_reg(GT9XXX_GSTID_REG, &i, 1); /* 清标志 */          //清除标志位后才能感应下一个点是否按下  有点按下上下两个if
        }                                                                 //不同时执行

        if ((mode & 0XF) && ((mode & 0XF) <= g_gt_tnum))
        {
            temp = 0XFFFF << (mode & 0XF);  /* 将点的个数转换为1的位数,匹配tp_dev.sta定义 */ //如果mode & 0XF是5的话，temp=0x1111 1111 1110 0000 最后temp还要取反
            tempsta = tp_dev.sta;           /* 保存当前的tp_dev.sta值 */
            tp_dev.sta = (~temp) | TP_PRES_DOWN | TP_CATH_PRES;//TP_PRES_DOWN:是否有按键按下  TP_CATH_PRES:按下/松开
            tp_dev.x[g_gt_tnum - 1] = tp_dev.x[0];  /* 保存触点0的数据,保存在最后一个上 */
            tp_dev.y[g_gt_tnum - 1] = tp_dev.y[0];

            for (i = 0; i < g_gt_tnum; i++)  //轮询的方式
            {
            ...
            res = 1;
            //防错   拆分为同时只触摸一个点，检测触摸的第一个点是否超界
            if (tp_dev.x[0] > lcddev.width || tp_dev.y[0] > lcddev.height)  /* 非法数据(坐标超出了) */
            {
                if ((mode & 0XF) > 1)   /* 有其他点有数据,则复第二个触点的数据到第一个触点. */ //第1、2个点为同一坐标时，排除第一个点的影响且第二个点不受影响
                {
    ...
    {
        if (tp_dev.sta & TP_PRES_DOWN)      /* 之前是被按下的 */  //TP_PRES_DOWN是宏定义0x80
        {
