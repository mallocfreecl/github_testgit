...
/*增加FCT零点标定有效范围 23.08.10 by zjj*/
#define FCT_TEST_ISHUNT_ZERO_MIN (ADC_SAMPLE_ISHUNT_ZERO_DEFAULT - (15 * 4))
#define FCT_TEST_ISHUNT_ZERO_MAX (ADC_SAMPLE_ISHUNT_ZERO_DEFAULT + (15 * 4))
...
static void Fct_CalIShuntZero(void);
    ...
    switch(UartDebugCmd)
    {
      ...
      case 2u: {
        ...
        FanComTest_WriteU8(3u, (ProductVersion.InnerVersion.reg >> 8u) & 0xFFu);
				...
				FanComTest_WriteU8(5u, ProductVersion.FctVersion); /*增加FCT版本号，24-8-26 by CL*/
        FanComTest_WriteU8(6u, 0u);   
				FanComTest_WriteU8(7u, 0u);     /*DEBUG版本号单独列出 24-8-26 by CL*/
      }
      break;
      case 3u: {
      ...
      /*增加效验和的检查 24-8-26 by CL*/
        ...
        break;
			/*增加DEBUG版本号 24-8-26 by CL*/
			case 0x0Bu: {
					FanComTest_WriteU8(1u, ProductVersion.DebugVersion);
					FanComTest_WriteU8(2u, 0u);
					FanComTest_WriteU8(3u, 0u);
					FanComTest_WriteU8(4u, 0u);
					FanComTest_WriteU8(5u, 0u);
					FanComTest_WriteU8(6u, 0u);
					FanComTest_WriteU8(7u, 0u);
      }
        break;
      default: /* 不做任何处理 */
        break;
  ...
  if(IsInFctTestMode == 1u)
  {
    ...
    switch(FctCmd)
    {
      case RX_ID_ENTRY_FCT_TEST_MODE:
      {
        ...
        FctTxBuf[6u] = 0x07u;
        FctTxBuf[7u] = 0x08u;
				/* Fct测试时进行电流零点标定，23.10.10 by lpy */
	       Fct_CalIShuntZero();
	}
      break;
      case RX_ID_TEST_MOS:
	...
	/*测试mos增加检测BDRV故障 23.02.09 by lxs*/
          ...
          switch(FrameCnt)
          {
            case 0u:
            {
              FrameCnt = 1u;
               ...
               /*ProductType由32位改为16位  2024-08-26-by CL*/
              ...
              FrameCnt = 4u;
                ...
                FctTxBuf[6u] = (((u8)((ECUFaults.bit.Ram != 0u) || (ECUFaults.bit.DFlash != 0u)))<<7u) 
                             ...
                             | ((u8)(ECUFaults.bit.ADC1ResultInvalid != 0u)<<3u)
							               | ((u8)(BdrvState != 0u)<<2u);
                FctTxBuf[7u] = ProductVersion.FctVersion;   /*改为FCT版本号 24-8-26 by CL*/
            }
            break;
            case 4u: 
			      {
		...
		/* 根据故障状态返回故障码，23.07.10 by zjj */
		if (ADCResults.ErrFlags.IShuntZero != 0u)
		{
			ErrorCode |= (1u << 0);				
		}
		/* 如果电流零点没有成功存入DFlash,则反馈错误。23.07.05 by zjj */
    ...
    /*更改LIN通信测试判断位置，防止主机在接收报文时，接收到未更新的数据 24/8/26 by CL*/
    /*测试LIN通信是否正常，接收到5A则返回 00 0A FF 0A 23.10.08 by lpy*/
    if(l_u16_rd_LI0_D_EWP_Setspeed() == TEST_MODE_LIN_TEST_RECEIVE_DATA)
    {
      l_u8_wr_LI0_Setspeed(TEST_MODE_LIN_TEST_BACK_DATA1);
      l_u8_wr_LI0_Actual_speed(TEST_MODE_LIN_TEST_BACK_DATA);
      l_u8_wr_LI0_Voltage(TEST_MODE_LIN_TEST_BACK_DATA2);
      l_u8_wr_LI0_Temperature(TEST_MODE_LIN_TEST_BACK_DATA);
    }
    else
    {
      l_u8_wr_LI0_Setspeed(0);
      l_u8_wr_LI0_Actual_speed(0u);
      l_u8_wr_LI0_Voltage(0);
      l_u8_wr_LI0_Temperature(0u);
    }
  }
  else
  {
...
/* BEGIN_FUNCTION_HDR
********************************************************************
* Function Name:Fct_CalIShuntZero
* Description:
*  fct零点标定及判断 
*   
*   
...
static void Fct_CalIShuntZero(void)
{
	static u8 Fct_CalIShuntZeroComplete = 0u;
		...
		if (Fct_CalIShuntZeroComplete == 0u)
		{
			...
			ADCResults.ErrFlags.IShuntZero = (Emo_calcIShuntZero() != 0u);
			if (ADCResults.ErrFlags.IShuntZero == 0u)
			{
				ADCResults.ErrFlags.IShuntZero = (ADCResults.RawAD.IShuntZero > FCT_TEST_ISHUNT_ZERO_MAX) ||
                                           (ADCResults.RawAD.IShuntZero < FCT_TEST_ISHUNT_ZERO_MIN);
				if (ADCResults.ErrFlags.IShuntZero == 0u)
				{
