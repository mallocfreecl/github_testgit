...
typedef volatile union {
  ...
  struct
  {
    ...
    unsigned bUsbReadyToDecode : 1; /* FreeMASTER packet is received, ready to decode in Poll function in Short
                                       Interrupt mode */
#endif
  } flg;
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @brief    Handle SCI communication (both
                                                                              *TX and RX)
                                                                              *
                                                                              * This function checks the SCI flags and
                                                                              *calls the Rx and/or Tx functions
                                                                              *
                                                                              * @note This function can be called either
                                                                              *from SCI ISR or from the polling routine
                                                                              *
                                                                              ******************************************************************************/
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @brief    Flush one communication JTAG
                                                                              *word
                                                                              *
                                                                              * This function gets the 4 bytes from
                                                                              *FreeMASTER TX buffer and place them to
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @brief    Handle JTAG communication
                                                                              *(both TX and RX)
                                                                              *
                                                                              * This function checks the JTAG flags and
                                                                              *calls the Rx and/or Tx functions
                                                                              *
                                                                              * @note This function can be called either
                                                                              *from JTAG ISR or from the polling routine
                                                                              *
                                                                              ******************************************************************************/
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @brief    MQX communication interface
                                                                              *initialization
                                                                              *
                                                                              ******************************************************************************/
...
static void FMSTR_USB_CDC_Callback(FMSTR_U8 controller_ID, FMSTR_U8 event_type, void *val);
static void FMSTR_USB_CDC_Notify_Callback(FMSTR_U8 controller_ID, FMSTR_U8 event_type, void *val);

/**************************************************************************/ /*!
                                                                              *
                                                                              * @brief    USB CDC communication
...
static FMSTR_BOOL FMSTR_InitUSB(void)
{
  ...
  error = USB_Class_CDC_Init(FMSTR_USB_CDC_ID, FMSTR_USB_CDC_Callback, NULL, FMSTR_USB_CDC_Notify_Callback, TRUE);
  if (error != USB_OK)
  {
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @brief    Handle USB CDC class periodic
                                                                              *task and initialize FreeMASTER TX packet
                                                                              *
                                                                              * This function perform USB CDC periodic
                                                                              *task. In Short Interrupt mode also decode
                                                                              * received FreeMASTER packet and
                                                                              *initialize transmit
                                                                              *
                                                                              ******************************************************************************/
static void FMSTR_ProcessUSB(void)
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @brief    Get data from FreeMASTER TX
                                                                              *buffer and send the data to USB device
                                                                              *
                                                                              * This function checks the FreeMASTER
                                                                              *transmit state and when FreeMASTER is
                                                                              * ready to send response to PC, copy
                                                                              *FreeMASTER TX data to local buffer and
                                                                              * pass down the buffer to USB CDC TX
                                                                              *function.
                                                                              *
                                                                              ******************************************************************************/
...
static void FMSTR_USB_CDC_Callback(FMSTR_U8 controller_ID, /* [IN] Controller ID */
...
static void FMSTR_USB_CDC_Notify_Callback(FMSTR_U8 controller_ID, /* [IN] Controller ID */
...
uint8_t pcm_usbCdcCoding[LINE_CODING_SIZE] = {(LINE_CODE_DTERATE_IFACE >> 0) & 0xFF,
...
static uint8_t FMSTR_USB_CDC_Notify_Callback(uint8_t event, uint16_t value, uint8_t **data, uint32_t *size, void *arg);
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @brief    Start listening on a serial
                                                                              *line
                                                                              *
                                                                              * Reset the receiver machine and start
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @brief    Send response of given error
                                                                              *code (no data)
                                                                              *
                                                                              * @param    nErrCode - error code to be
                                                                              *sent
                                                                              *
                                                                              ******************************************************************************/
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @brief    Finalize transmit buffer
                                                                              *before transmitting
                                                                              *
                                                                              * @param    nLength - response length (1
                                                                              *for status + data length)
                                                                              *
                                                                              *
                                                                              * This Function takes the data already
                                                                              *prepared in the transmit buffer
                                                                              * (inlcuding the status byte). It computes
                                                                              *the check sum and kicks on tx.
                                                                              *
                                                                              ******************************************************************************/
...
/**************************************************************************/ /*!
                                                                              ...
                                                                              *
                                                                              * @param  pTxChar  The character to be
                                                                              *transmit
                                                                              *
...
/**************************************************************************/ /*!
                                                                              ...
                                                                              *
                                                                              * @param  nRxChar  The character to be
                                                                              *processed
                                                                              *
                                                                              * Handle the character received and -if
                                                                              *the message is complete- call the
...
FMSTR_BOOL FMSTR_Rx(FMSTR_BCHR nRxChar)
{
  ...
  {
#if DEBUG_DIAG_OUT_FREE_MASTER_RECERIVE
  ...
  if (pflg->flg.bRxLastCharSOB)
  {
    ...
    *pcm_pRxBuff++ = nRxChar;
    ...
    if (!((~nRxChar) & FMSTR_FASTCMD))
    {
      ...
      pcm_nRxTodo =
        (FMSTR_SIZE8)(((((FMSTR_SIZE8)nRxChar) & FMSTR_FASTCMD_DATALEN_MASK) >> FMSTR_FASTCMD_DATALEN_SHIFT) + 1U);
    }
