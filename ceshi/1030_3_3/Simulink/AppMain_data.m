...
TaskTimeBase.PwmIn = 5e-3;
TaskTimeBase.SpeedCtrl= 5e-3;
TaskTimeBase.FaultDiag = 5e-3;
...
CtrlConst.PwmIn.BusTimeOutCntTHR = uint16(500); %Bus idle State time out time,unit:ms

% Speed Ctrl by pwm in
CtrlConst.SpeedCtrl.PwmIn = struct;
CtrlConst.SpeedCtrl.PwmIn.PowerOnBlankTime = uint16(100e-3/TaskTimeBase.SpeedCtrl);

CtrlConst.SpeedCtrl.PwmIn.Freq = struct;
CtrlConst.SpeedCtrl.PwmIn.Freq.Min = uint16(500);%0.1Hz
CtrlConst.SpeedCtrl.PwmIn.Freq.Max = uint16(10000);%0.1Hz
CtrlConst.SpeedCtrl.PwmIn.Freq.Tolerance = uint16(10);%0.1Hz
...
CtrlConst.SpeedCtrl.PwmIn.Duty.Default = uint16(50); % 50*0.1%=5%
CtrlConst.SpeedCtrl.PwmIn.Duty.FreqError = uint16(1000); % 1000*0.1%=100% 错误频率上电恢复到正常频率会出现先停止后启动到期望转速
                                                         %     该修改方案适用于频率错误为全速命令的项目
...
CtrlConst.SpeedCtrl.PwmIn.TargetSpeed.Min = int16(3100);%rpm
CtrlConst.SpeedCtrl.PwmIn.TargetSpeed.Max = int16(6000);%rpm
...
CtrlConst.SpeedCtrl.PwmIn.TargetSpeed.DelayOutTime.FreqInvalid = uint16(0/TaskTimeBase.SpeedCtrl);%
...
CtrlConst.FaultDiag.Voltage.THR.LowEntry = int16(150); % 0.1V
CtrlConst.FaultDiag.Voltage.THR.LowExit = int16(160-3); % 0.1V
CtrlConst.FaultDiag.Voltage.THR.HighEntry = int16(365-3); % 0.1V
CtrlConst.FaultDiag.Voltage.THR.HighExit = int16(320); % 0.1V

CtrlConst.FaultDiag.AmbTemperature = struct;
CtrlConst.FaultDiag.AmbTemperature.FilterTime = uint16(2/TaskTimeBase.FaultDiag);
CtrlConst.FaultDiag.AmbTemperature.LongFilterTime = uint16(10/TaskTimeBase.FaultDiag);
