...
#define EcuFaultDiag_PERIOD_TASK_TIME_BASE (25u)/* ms */     /*增加宏定义，修改时基为25ms，24-06-24 by CL*/
#define Ref5VUnderVoltageCnt_THR (500/EcuFaultDiag_PERIOD_TASK_TIME_BASE) /*增加Ref5VUnderVoltageCnt次数阈值，24-06-24 by CL*/
...
void SDK_EcuFaultDiag(void)
{
  ...
  else
  {
	...
	if(Ref5VUnderVoltageCnt < Ref5VUnderVoltageCnt_THR)	  /*增加宏定义，次数阈值由50到20 24-06-19 by CL*/
	{	
  ...
  if(BlankTime < 1000u)
  {
    BlankTime += EcuFaultDiag_PERIOD_TASK_TIME_BASE;  /*增加宏定义，修改时基为25ms，24-06-19 by CL*/
  }
  else
  {
    u8 BDRVLow;
...
void SDK_Mem_Check(void)
{
	...
	ECUFaults.bit.Ram = SDK_RAM_Check();
	 Sdk_exitCritical();
	ECUFaults.bit.CFlash = 0;//SDK_FLASH_Check();
  ECUFaults.bit.DFlash = SDK_NVM_ECC_Check();
}
