...
void AppMain_step(void)
{
  ...
  if (M->Timing.TaskCounters.TID[1] == 0)
  {
    ...
    ECUFaultDiag(U.SupVoltage, U.AmbTemperature, U.ChipFault, &B.ECUFault,
                 &B.Status_ila3, &B.Status_jbt3, &DW.ECUFaultDiag_ima5);
  ...
  /* Chart: 'AmbTPreOverDiag' (':2333') incorporates:
   *  Inport: 'AmbTemperature' (':767')
   */
  AmbTOverDiag(U.AmbTemperature, &B.Status, 200, 145, 143, -40, -38,
  ...
  if ((int32_T)B.Status > 0)
  {
     ...
     */
    if ((int32_T)rtb_Switch_jqwg > 2550)
    {
      /* Outport: 'TargetSpeedLimited' (':2216') */
      Y.TargetSpeedLimited = 2550;
    }
    else
    {
      /* Outport: 'TargetSpeedLimited' (':2216') */
...
void AppMain_initialize(void)
{
  ...
  ECUFaultDiag_Init(&B.Status_ila3, &B.Status_jbt3, &DW.ECUFaultDiag_ima5);
  ...
  /* SystemInitialize for Chart: 'AmbTPreOverDiag' (':2333') */
